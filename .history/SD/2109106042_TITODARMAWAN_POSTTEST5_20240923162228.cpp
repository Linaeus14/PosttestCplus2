...
void DP(Node *&head, Node *&tail, int &jumlahRoster)
{
    ...
    cout << "Hapus urutan ke "
         << "(1-" << jumlahRoster << ") : ";
    cin >> input;
    if (0 < input && input < jumlahRoster + 1)
    {
        if (input == 1)
        {
        ...
        temp2 = temp->next->next; // Node after the one to delete

        if (temp->next == tail) // Deleting the last node
        {
            tail = temp;
            tail->next = nullptr;
        }
        ...
        {
            temp->next = temp2; // Skip over the node to delete
            if (temp2 != nullptr)
            {
                temp2->prev = temp; // Update temp2's prev pointer
            }
        }
